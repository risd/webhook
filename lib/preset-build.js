...
module.exports = function PresetBuild (options) {
  ...
  var config = {
    ...
    realData: {},
    settingsData: {},
    pathToWrite: options.toFile || ".preset-data.json",
  ...
  async.series([
      ...
      if(options.all) {
        firebaseRoot.child('buckets/' + config.confData.siteName + '/' + config.confData.secretKey + '/dev/data').once('value', function(snap) {
          ...
          firebaseRoot.child('buckets/' + config.confData.siteName + '/' + config.confData.secretKey + '/dev/settings').once('value', function(snap) {
            ...
            firebaseRoot.child('buckets/' + config.confData.siteName + '/' + config.confData.secretKey + '/dev/deploys').once('value', function(snap) {
              config.deploys = snap.val() || {};
              step();
            }, function(err) { step(); });
          }, function(err) { step(); });
        }, function(err) { step(); });
    ...
    if(!options.all) {
      ...
      fs.writeFileSync(config.pathToWrite, typeDataString, {'flags': 'w+'});
    } else {
      var dataObj = {
      ...
      fs.writeFileSync(config.pathToWrite, dataString, {'flags': 'w+'});
    }
    process.exit(0);
  });
};
