...
module.exports = function ( options, callback ) {
  ...
  var config = {
    ...
    webhookPassword: '',
    branch: options.branch || gitBranch.sync() || Deploys.utilities.defaultBranch()
  };
  ...
  async.series( series, function onComplete () {
	...
	function getDeployConfiguration ( step ) {
		var forSite = { siteName: config.sitename, key: config.sitekey };
	...
	function setDeployConfiguration ( step ) {
		var setterOptions = {
			...
			key: config.sitekey,
			deploy: { branch: config.branch, bucket: config.bucket } };
	...
	function stepForLogConfiguration ( step ) {
		return function logConfiguration ( error, deployConfiguration ) {
			...
			console.log( 'Deploys:\n---' )
			deployConfiguration.deploys.forEach( function displayDeployConfiguration ( deploy ) {
